<script>
  ...
  const lobbyFromList = $derived($lobbyStore.find((item) => item.id === lobbyId));

  const lobby = $derived(
    $currentLobby && $currentLobby.id === lobbyId ? $currentLobby : lobbyFromList
  );

  const isMember = $derived(
    lobby && $userStore && lobby.players.some((player) => player.id === $userStore.id)
  );
  ...
  const isLeader = $derived(lobby && $userStore && lobby.owner_id === $userStore.id);
...
{:else}
  <section class="lobby-page page">
    <div class="lobby-page__back-row">
      <Button text="Back to lobbies" icon="←" onClick={() => navigate("/lobbies")} />
    </div>
    <LobbyActionsBar
      {lobby}
      {isMember}
    ...
    {#if isLeader && isEditing}
      <div class="lobby-page__leader-card">
        <div class="lobby-page__leader-header">
          ...
          <div>
            <Button text="Close" onClick={handleCancelEdit} class="lobby-page__close" />
          </div>
        </div>
        ...
        <div class="lobby-page__actions">
          <button class="lobby-page__save" onclick={handleSaveSettings}> Save changes </button>
        </div>
      </div>
    {/if}
